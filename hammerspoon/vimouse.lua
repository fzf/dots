...
--
-- h/j/k/l moves the mouse cursor by 10 pixels.  Holding shift moves by 100
  ...
  tap = hs.eventtap.new({eventTypes.keyDown, eventTypes.keyUp}, function(event)
    ...
    elseif event:getType() == eventTypes.keyDown then
      ...
      if is_tapkey or code == keycodes['escape'] then
        ...
        return true
      elseif (code == keycodes['h'] or code == keycodes['t']) and flags.ctrl then
        if repeating ~= 0 then
      ...
      elseif code == keycodes['d'] then
        x_delta = step * mul * -1
      elseif code == keycodes['n'] then
        x_delta = step * mul
      elseif code == keycodes['h'] then
        y_delta = step * mul
      elseif code == keycodes['t'] then
        y_delta = step * mul * -1
      end
